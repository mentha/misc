...
#ifndef UINPUT_MANAGED
static int open_uinput(void)
{
	int fd = open(UINPUTPATH, O_RDONLY);
	if (fd >= 0)
		return fd;
	if (errno != ENOENT) {
		LOG(FATAL, "cannot open " UINPUTPATH ": %s\n", errstr);
		return -1;
	}

	if (system(UINPUTLOADMODULE)) {
		LOG(FATAL, "failed loading uinput module\n");
		return -1;
	}

	fd = open(UINPUTPATH, O_RDONLY);
	if (fd < 0)
		LOG(FATAL, "cannot open " UINPUTPATH ": %s\n", errstr);
	return fd;
}
...
static int start_daemon(void)
{
	...
	int uinputfd = -1;
#ifndef UINPUT_MANAGED
	uinputfd = open_uinput();
	if (uinputfd < 0) {
		LOG(FATAL, "cannot create uinput device\n");
		goto exit_close_kbddev;
	}
#endif
	...
	int uinputfnfd = -1;
#ifndef UINPUT_MANAGED
	uinputfnfd = open_uinput();
	if (uinputfnfd < 0) {
		LOG(FATAL, "cannot create uinput device\n");
		goto exit_close_uinputdev;
	}
#endif
